...
import { Node, Filter, TransactionFromArchiver, TransactionFromExplorer, NodeJSError, ReceiptFromExplorer, WrappedDataContractStorage, IpData, ToData, FromData, OriginalTxData, AccountTypesData, Account2 } from './types'
...
export class RequestersList {
  ...
  checkAndBanSpammers(): void {
    ...
    for (const abusedData of mostAbusedSorted) {
      ...
      const sortedCallers: FromData[] = Object.values(abusedData.from).sort((a: FromData, b: FromData) => b.count - a.count)
    ...
    if (reqType === 'eth_sendRawTransaction') {
      try {
        ...
        if (
          ...
          if (config.recordTxStatus)
            createRejectTxStatus(bufferToHex(transaction?.hash() as Buffer), 'Rejected by JSON RPC rate limiting', ip)
          return false
        }
...
export async function fetchQueryExpb(query: string, maxRetries = 6): Promise<AxiosResponse<{ transactions: TransactionFromArchiver }> | null> {
...
export async function fetchTxReceipt(explorerUrl: string, txHash: string, hashReceipt = false): Promise<TransactionFromExplorer | ReceiptFromExplorer> {
...
async function fetchAccountFromExplorer(explorerUrl: string, key: string, timestamp: number): Promise<{ accountId: string; data: AccountTypesData } | undefined> {
...
async function fetchAccountFromArchiver(key: string, timestamp: number): Promise<{ accountId: string; data: AccountTypesData } | undefined> {
  const res = await requestWithRetry(
    RequestMethod.Get,
...
async function fetchLatestAccount(key: string, type: number): Promise<{ accountId: string; data: AccountTypesData } | undefined> {
...
async function fetchAccountFromCollector(account: { type: number; key: string }, timestamp: number): Promise<{ accountId: string; data: AccountTypesData } | undefined | null> {
  if (account.type === 0) {
    // EOA/CA
    ...
    const res = await serviceValidator.getAccount(account.key)
    if (!res.data.accounts) {
      result = {
...
async function fetchAccount(account: { type: number; key: string }, timestamp: number): Promise<{ accountId: string; data: AccountTypesData } | undefined | null> {
  if (account.type === 0) {
    // EOA/CA
