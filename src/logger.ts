...
export async function saveInterfaceStat() {
  ...
  try {
    ...
    for (let i = 1; i < apiPerfLogData.length; i++) {
      let { api_name, tfinal, timestamp, nodeUrl, success, reason, hash } = apiPerfLogData[i]
...
export async function txStatusSaver(_txs: DetailedTxStatus[]) {
  ...
  if (bulkInsertTxs) {
    ...
    /**
     * Prepares a SQL statement for bulk insertion of transaction status information into the database. 
     * For each transactions, the SQL statement contains a placeholder for each column value.
     */
